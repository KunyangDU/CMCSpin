...
function _minimum_cell(Latt::AbstractLattice, ψ::SimpleState)
    Lx,Ly = size(Latt)
    lspq = [[nx//Lx,ny//Ly] for nx in 0:Lx-1 for ny in 0:Ly-1]
    kbs = kbasis2(Latt)
    lstk = map(x -> Tuple(x' * kbs),lspq)
    SM = ψ.pattern' * ψ.pattern
    SSF =  FT2(Latt,SM,lstk)
    pq = lspq[findmax(SSF)[2]]
    return lspq,SSF,pq
end

_conv_cell(Latt::AbstractLattice, ψ::SimpleState) = denominator.(_minimum_cell(Latt,ψ)[3])
_prim_cell(Latt::AbstractLattice, ψ::SimpleState) = _magnetic_cell_size(_minimum_cell(Latt,ψ)[3])
_conv_prim_cell(Latt::AbstractLattice, ψ::SimpleState) = _minimum_cell(Latt,ψ)[3] |> x -> (denominator.(x), _magnetic_cell_size(x))

function minimum_pattern(Latt::AbstractLattice, ψ::SimpleState)
    n1,n2 = _conv_cell(Latt,ψ)
    Latt′ = PeriSqua(n1,n2)
    coors = map(x -> coordinate(Latt′,x), 1:length(Latt′))
    ψ′ = SimpleState(Latt′,zeros)
    for i in 1:length(Latt)
        shift = Latt.unitcell.lattice_vecs * [n1*div(Latt[i][2][1],n1), n2*div(Latt[i][2][2],n2)]
        R₀ = coordinate(Latt,i) - shift 
        ϵ,id = findmin(x -> norm(x - R₀), coors)
        @assert ϵ ≈ 0
        ψ′[id] += ψ[i] * n1 * n2 / Latt.lattice.N
    end
    return Latt′, ψ′
end
