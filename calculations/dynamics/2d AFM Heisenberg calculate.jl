...
for i in 1:20
    ...
    ψ = Dict(
        "S" => [[0,0,(-1)^sum(Latt[i][2])] + 0.03*vcat(randn(2),0) |> x -> x/norm(x) for i in 1:length(Latt)]
    )
    ...
    for (iq,q) in enumerate(lsq)
        eq = [exp(1im * dot([q,pi - 0.1], coordinate(Latt,i))) for i in 1:length(Latt)] / sqrt(length(Latt))
        Smq0 = sum(eq' .* lsS[1])
