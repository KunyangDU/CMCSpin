using JLD2,LatticeUtilities,CairoMakie,LinearAlgebra,Statistics
...
for i in 1:Neff - Ncut
    ...
    lsM[i] = norm(sum([total_data[i + Ncut]["S"][(j,)] for j in 1:length(Latt)]))
end
...
lsEerr = [std(lsEaccu[1:i] / length(Latt)) for i in 1:Neff - Ncut]
lsMerr = [std(lsMaccu[1:i] / length(Latt)) for i in 1:Neff - Ncut]
...
display(fig)
(lsE / length(Latt)) |> x -> maximum(x) - minimum(x)
maximum(lsE)/length(Latt)